...
(function () {
  ...
  var JS_WS_CLIENT_VERSION = '0.2.0'
    ...
    var onclose = function (event) {
      ...
      if (!!params.reconnect) {
        if(reconnectAttempts < maxReconnectAttempts) {
          reconnect = true
          var reconnectionDelay = reconnectAttempts * 2
          pomelo.emit('disconnect', { delay: reconnectionDelay, attempt: reconnectAttempts })
          reconncetTimer = setTimeout(function () {
            pomelo.emit('reconnecting', { attempt: reconnectAttempts })
            connect(params, reconnectUrl, cb)
          }, reconnectionDelay * 1000)
          reconnectAttempts++
        } else {
          pomelo.emit('disconnect', false)
        }
      }
    }
  ...
  pomelo.disconnect = function (code) {
    if (socket) {
      if (code !== 1001) {
        reconnectAttempts = 99 // disable reconnect
      }
